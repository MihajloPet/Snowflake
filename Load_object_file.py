CREATE OR REPLACE PROCEDURE BUSDATA.SF.LOAD_OBJECT_FILE("DEST_DATABASE" VARCHAR(16777216), "DEST_SCHEMA" VARCHAR(16777216), "OBJECT_NAME" VARCHAR(16777216))
RETURNS TABLE ()
LANGUAGE PYTHON
RUNTIME_VERSION = '3.8'
PACKAGES = ('snowflake-snowpark-python')
HANDLER = 'main'
COMMENT='this procedure will load use snowpark to get tlatest file from AWS S3 bucket salesforce-objects-data-raw, add columns if missing, add columns to Snowpark table if needed and load data to snowflake table'
EXECUTE AS OWNER
...
def elt_s3_file(session, table_name, stage_path, object_element_file,file_to_load,table_name_ext):
    ...
    for key in unique_columns_type_dict:
        ...
        # elif column_dtype_snowflake==''TIMESTAMP_TZ(9)'':
...
def main(session: snowpark.Session,dest_database,dest_schema,object_name): 
